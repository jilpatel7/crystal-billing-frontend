...
export const markAttendance = async (data: AttendanceFormData) => {
  const { data: attendance } = await axiosInstance.post('/company-staff/attendance/mark', data);
  return attendance;
};

export const getAttendance = async (params: { staff_id?: number; date?: string; month?: string; year?: string }) => {
  const { data } = await axiosInstance.get('/company-staff/attendance/get', { params });
  return data;
};

export const updateAttendance = async (id: number, data: Partial<AttendanceFormData>) => {
  const { data: attendance } = await axiosInstance.put(`/company-staff/attendance/update/${id}`, data);
  return attendance;
};

export const requestLeave = async (data: LeaveRequestFormData) => {
  const { data: leave } = await axiosInstance.post('/company-staff/attendance/leave-request', data);
  return leave;
};

export const getStaffAttendanceSummary = async (staff_id: number, month: number, year: number) => {
  const { data } = await axiosInstance.get(`/company-staff/attendance/summary`, {
